public class ListDL {
    ...
    private static void __Insert(ListDL cell, ListDL prev, ListDL next) {   // prevとnextの間にcellを挿入
        prev.next = cell;
        cell.prev = prev;
        cell.next = next;
        next.prev = cell;
    }
    private static void __Delete(ListDL prev, ListDL next) {                // prevとnextの間のセルを削除
        prev.next = next;
        next.prev = prev;
    }
